    ...
    var grid = new Ext.grid.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
			...
			columns:[new Ext.grid.RowNumberer(),
				{header: '药品编号', width: 100, sortable:true, dataIndex: 'spid'},
	            {header: '药品名称', width: 130, sortable:true, dataIndex: 'spname'},
	            {header: '药品型号', width: 80, sortable:true, dataIndex: 'xinghao'},
        ...
        tbar:[{
        	...
        	handler: function(){
        		var record= grid.getSelectionModel().getSelected();
				if(!record){
                	Ext.Msg.alert('信息提示','请选择要删除的药品');  
				}else{
					Ext.MessageBox.confirm('删除提示', '是否删除该药品？', function(c) {
					   if(c=='yes'){
					   		Ext.Ajax.request({
					   		...
					   		Ext.Ajax.request({
					   			...
					   			success : function(o) {
					   				if(o.responseText=="false"){
					   					Ext.Msg.alert("信息提示","该类别有药品信息不能删除");
					   				}else{
						   				v_lbid = "0";     //设为默认节点
						   				v_lbname = "";
